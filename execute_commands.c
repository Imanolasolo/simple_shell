			...
			char *envp[], char *exec_name)
{
	...
	cur_node = disequence(command_q); /* get first command */
 ...
 * @command:			Command_t pointer that contains command to
 *						check executability of.
 * Return:				1 can execute or 0 can't execute.
 */
 ...
 * Return:				-1 if it is not custom. Index in customs[]
 *						if it is custom, (Will be >= 0 in this case)
 */
 ...
 * Return:				1 executed successfully or 0 failed execution.
 */
				...
				sequence_t *seq)
{
	...
	case 'e':
		...
		case 'x': /* check exit */
			...
			exit_shell(his_q, seq, exit_status, envp);
			break;
 ...
 *						0 on success or error code on fail.
 */
